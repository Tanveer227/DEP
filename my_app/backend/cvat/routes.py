...
labels = [label for label in DATASET_INFO["labels"] if label != "background"]
# ---------------------------- Helper Functions ----------------------------
...
def get_segmentation(mask, label_id):
    """Generate CVAT-compatible polygon coordinates with validation."""
    binary_mask = (mask == label_id).astype(np.uint8)
    contours, _ = cv2.findContours(binary_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    valid_segments = []
    for contour in contours:
        if len(contour) >= 3:  # Minimum 3 points for polygon
            flattened = contour.flatten().tolist()
            # Ensure even number of coordinates and minimum 6 elements (3 points)
            if len(flattened) % 2 == 0 and len(flattened) >= 6:
                valid_segments.append(flattened)
...
def generate_coco_annotations(result_png_dir):
    """Convert segmentation PNGs to COCO format with dynamic label mapping."""
    # Collect all unique labels across all images
    label_values = set()
    
    # First pass: collect all unique label values (excluding 0)
    for png_file in os.listdir(result_png_dir):
        if not png_file.lower().endswith('.png'):
            continue
        mask = cv2.imread(os.path.join(result_png_dir, png_file), cv2.IMREAD_GRAYSCALE)
        label_values.update(np.unique(mask))
    
    # Remove background (0) and sort remaining labels
    label_values.discard(0)
    sorted_labels = sorted(label_values)
    
    # Create dynamic label mapping
    label_map = {value: idx+1 for idx, value in enumerate(sorted_labels)}
    ...
    # Second pass: process images with dynamic labels
    for image_id, png_file in enumerate(sorted(os.listdir(result_png_dir))):
        if not png_file.lower().endswith('.png'):
            continue
        ...
        # Process annotations using dynamic labels
        for original_value in np.unique(mask):
            if original_value == 0:
                continue
                
            # Map to dynamic label ID
            label_id = label_map[original_value]
            
            # Get segmentation and bbox
            segmentation = get_segmentation(mask, original_value)
            bbox = get_bounding_box(mask, original_value)
            ...
            if segmentation and bbox:
                coco_data["annotations"].append({
                    ...
                    "area": float(np.sum(mask == original_value)),
                    "iscrowd": 0
                })
                annotation_id += 1
    ...
    try:
        ...
        for nifti_id in nifti_ids:
            try:
                ...
                coco_data = generate_coco_annotations(result_png_dir)
